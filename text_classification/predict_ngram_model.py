...
def predict_ngram_model(sentences, model_dir, learning_rate=1e-3):
    ...
    vectorizer = pickle.load(open(os.path.join(model_dir, 'vectorizer.pickle'), "rb"))
    selector = pickle.load(open(os.path.join(model_dir, 'selector.pickle'), "rb"))
    x_predict = vectorizer.transform(tokenize_list)
    x_predict = selector.transform(x_predict)
    x_predict = x_predict.toarray()

    model = load_model(os.path.join(model_dir, 'cnews_mlp_model.h5'))
...
if __name__ == '__main__':
    ...
    predict_ngram_model([sentence1, sentence2], model_dir=FLAGS.model_dir)