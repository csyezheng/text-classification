...
import load_data
import pytest
from train_fine_tuned_sequence_model import train_fine_tuned_sequence_model
...
def test_train_ngram_model():
    data_dir = './data/'
    data = load_data.load_cnews_dataset(data_dir)
...
def test_train_sequence_model():
    data_dir = './data/'
    data = load_data.load_cnews_dataset(data_dir)
    acc, loss = train_sequence_model(data)
...
def test_train_fine_tuned_sequence_model():
    ...
    data = load_cnews_dataset(data_dir)
    acc, loss = train_fine_tuned_sequence_model(data, embedding_data_dir)
