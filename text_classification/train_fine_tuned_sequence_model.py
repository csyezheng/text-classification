...
Vectorizes training, validation and testing texts into sequences and uses that for
...
from text_classification.build_model import sepcnn_model
from text_classification.load_data import load_cnews_dataset
from text_classification.vectorize_data import sequence_vectorize
...
def _get_embedding_matrix_glove(word_index, embedding_data_dir, embedding_dim):
...
def _get_embedding_matrix_fasttext(word_index, embedding_data_dir, embedding_dim):
    """Gets embedding matrix from the embedding index data.

    # Arguments
        word_index: dict, word to index map that was generated from the data.
        embedding_data_dir: string, path to the pre-training embeddings (Word2Vec or FastText trained word vectors).
        embedding_dim: int, dimension of the embedding vectors.
    ...
    # References:
        ...
        Page 125 of the <<fastText Quick Start Guide>> book.
    """

    fname = os.path.join(embedding_data_dir, 'cc.zh.300.bin')
...
def train_fine_tuned_sequence_model(data,
    ...
    # Vectorize texts.
    x_train, x_val, x_test, word_index = sequence_vectorize(
            train_texts, val_texts, test_texts, model_dir)

    # Number of features will be the embedding input dimension. Add 1 for the
    # reserved index 0.
    num_features = min(len(word_index) + 1, TOP_K)

    embedding_matrix = _get_embedding_matrix_fasttext(
    ...
    # Save the model.
    model.save(os.path.join(model_dir, 'cnews_sepcnn_model_with_pre_trained_embedding.h5'))
    ...
    model.load_weights(os.path.join(model_dir, 'cnews_sepcnn_model_with_pre_trained_embedding.h5'))
    ...
    # Save model.
    model.save(os.path.join(model_dir, 'cnews_sepcnn_fine_tuned_model.h5'))
...
if __name__ == '__main__':
    ...
    # Using the cnews dataset to training sequence model.
    data = load_cnews_dataset(FLAGS.data_dir)
    train_fine_tuned_sequence_model(data, FLAGS.embedding_data_dir, FLAGS.model_dir)
